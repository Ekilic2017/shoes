...
const Card = ({item}:{item:Shoe}) => {
  ...
  return (
    <div className="flex flex-col justify-between">
      ...
      <Link to={`/detail/${item.id}`} className="bg-gray-dark text-white
       font-medium px-4 py-2 rounded-[8px] transition hover:bg-black text-center">
        Ürünü görüntüle - 
        <span className={item.discount ? "text-yellow" : "text-white"}>
          ${calcDiscount(item.price,item.discount)}</span>
          {item.discount && (
            <span className="max-xl:hidden line-through ps-2 text-yellow">
            (${item.price})
              </span>
             )}
        </Link>
