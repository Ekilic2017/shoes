...
const Filter = ({isOpen,close}:Props) => {
  ...
  const [size,setSize]=
  useState<string[]>(params.get("size")?.split(",")|| [])
  const [color,setColor]=
  useState<string[]>(params.get("color")?.split(",")|| [])
  const [gender,setGender]=
  useState<string[]>(params.get("gender")?.split(",")|| [])
  ...
  return (
    <div className={
      isOpen ?
      `"max-lg:fixed max-lg:inset-0 max-lg:grid
     max-lg:place-items-center max-lg:bg-zinc-900 max-lg:bg-opacity-60 
     z-10`:"max*lg:hidden"
     }>
          <div className="max-lg:max-w-lg max-lg:mx-auto max-lg:h-[80vh] max-lg:min-w-[400px]">
        ...
        <form className="max-lg:p-5 bg-gray h-full flex flex-col gap-6 rounded-b-md">
            ...
            <button type="reset" onClick={handleReset}
             className="border border-gray-dark p-2 rounded-lg hover:bg-gray-dark hover:text-white">Sıfırla</button>
            </form>
            </div>
    </div>
  )
