...
const Size = ({data}:{data:string}) => {
   ...
   const toggle = (num: string) => {
    if (selected === num) {
      // seçili numaraya tekrar tıklanırsa urldem kaldır
      setSelected("");
    } else {
      // seçili olmayan numaraya tıklanırsa urle ekler
      setSelected(num);
    }
  };
    ...
    <div> 
        ...
        {numbers.map((num)=>{
         const found = selected === num;
            return(
                <button onClick={()=>toggle(num)}
                 disabled={!stockSize.includes(num)}
                 className={`py-2 px-4 lg:px-0 text-center 
                    rounded-md cursor-pointer transition
                     hover:bg-zinc-400 disabled:bg-[#D2D1D3] 
                    disabled:text-[#8F8C91] ${found ? "bg-gray-dark text-white" : "bg-white"
